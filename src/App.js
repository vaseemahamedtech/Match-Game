...
class App extends Component {
  state = {
    ...
    randomItem: imagesList[Math.floor(Math.random() * imagesList.length)],
  }
  ...
  checkImageIsSame = id => {
    ...
    const isCorrect = randomItem.id === id
  ...
  render() {
    const {score, timer, activeTab, randomItem} = this.state
    const filteredImageList = this.getFilteredImage()
      ...
      <div className="app-container">
        <div className="nabar-container">
          ...
          <div className="score-container">
            <p className="score-design">
              Score:<span className="score-count-design"> {score}</span>
            </p>
            <img
            ...
            />
            <p className="timer-design">{timer} sec</p>
          </div>
        </div>
        ...
        <div className="main-container">
          {timer === 0 ? (
            <div className="result-container">
              ...
              />
              <p className="result-title">Your Score</p>
              <p className="result-title">{score}</p>
              <button
              ...
              >
                ...
                />
                <p className="result-button-name">PLAY AGAIN</p>
              </button>
            </div>
          ) : (
                ...
                {tabsList.map(eachTabItem => (
                  <li
                    className={`list-container ${
                      activeTab === eachTabItem.tabId ? 'active-tab-design' : ''
                    }`}
                    key={eachTabItem.tabId}
                    onClick={() => this.setActiveTabId(eachTabItem.tabId)}
                  >
                    <p className="tab-design">{eachTabItem.displayText}</p>
                  </li>
                ))}
              </ul>

              <ul className="item-unordered-list">
                {filteredImageList.map(eachImageItem => (
                  <li
                    className="item-image-design-list"
                    key={eachImageItem.id}
                    onClick={() => this.checkImageIsSame(eachImageItem.id)}
                  >
                    <img
                      src={eachImageItem.thumbnailUrl}
                      alt="thumbnail"
                      className="item-liste-image"
                    />
                  </li>
                ))}
              </ul>
